...
export default function ExamDetailTabs({ tabs, importantDates, slug, defaultTab }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-line" style={{ overflow: 'clip' }}>
        ...
        {tabs.map(tab => {
          ...
          return (
            ...
            >
              ...
              {isActive && <span className="block w-4 h-0.5 rounded-full bg-primary mt-0.5" />}
            </Link>
          );
        })}
      </div>
    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-primary-tint border-b border-line">
          ...
          <tbody className="divide-y divide-gray-100">
            {dates.map((d, i) => (
              <tr key={i} className="hover:bg-primary-tint/40 transition-colors">
                <td className="py-3.5 px-5 text-gray-400 text-xs font-medium">{i + 1}</td>
                <td className="py-3.5 px-5 text-gray-700">{d.label}</td>
                <td className="py-3.5 px-5 font-semibold text-primary-dark whitespace-nowrap">{d.date}</td>
              </tr>
            ))}
          </tbody>
        </table>
